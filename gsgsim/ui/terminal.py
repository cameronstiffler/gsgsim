...
WHITE="\033[37m";CYAN="\033[36m";GREY="\033[90m";RED="\033[31m";YELLOW="\033[33m";RST="\033[0m"
...
def rank_icon(c): r=getattr(c,"rank",None); return " ⭐" if r==Rank.SL else " 🔶" if r==Rank.SG else " 💪" if r==Rank.TITAN else ""

def prop_icons(c):
    props=getattr(c,"properties",{}) or {}; out=[]
    if props.get("resist"): out.append(" ✋")
    if props.get("no_unwind"): out.append(" 🚫")
    return "".join(out)

def cost_str(c):
    w=int(getattr(c,"deploy_wind",0) or 0); g=int(getattr(c,"deploy_gear",0) or 0); m=int(getattr(c,"deploy_meat",0) or 0)
...
def ability_lines(c:Card)->List[str]:
    ...
    for j,a in enumerate(getattr(c,"abilities",[]) or []):
        ...
        w=int(getattr(a,"wind_cost",0) or 0); g=int(getattr(a,"gear_cost",0) or 0); m=int(getattr(a,"meat_cost",0) or 0)
...
class TerminalUI:
    ...
    def run_loop(self, gs:GameState):
        ...
        while True:
            if engine_rule_shim and engine_rule_shim.check_sl_loss(gs): break
            self.render(gs)
