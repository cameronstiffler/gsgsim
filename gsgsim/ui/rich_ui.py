...
class RichUI:
    ...
    def render(self, gs: GameState):
        # self.console.clear()  # optional; keep off until stable visibility
        ...
        p1, p2 = gs.p1, gs.p2
        self.console.print(
            f"[bold]P1 {p1.name}[/bold]: board={len(p1.board)} hand={len(p1.hand)}    |    [bold]P2 {p2.name}[/bold]: board={len(p2.board)} hand={len(p2.hand)}"
        )
    ...
    def run_loop(self, gs: GameState):
        ...
        try:
            while True:
                ...
                if cmd in {"help", "?"}:
                    self.console.print(
                        "commands: help | quit(q) | end(e) | deploy(d) <hand_idx> | use(u) <src_idx> <abil_idx> [tgt_idx]"
                    )
                    continue
                if cmd in {"end", "e"}:
                    end_of_turn(gs)
