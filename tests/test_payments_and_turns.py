...
def test_transactional_wind_no_partial():
    ...
    expensive = Card(name="BigBoy", rank=Rank.BG)
    expensive.deploy_wind = 5
...
def test_retire_at_4_wind_during_payment():
    gs = mk_state()
    payer = Card(name="Grim", rank=Rank.SL)
    c = Card(name="Voidling", rank=Rank.BG)
    c.deploy_wind = 4
    gs.turn_player.board.append(payer)
...
def test_new_this_turn_cannot_pay():
    ...
    b = Card(name="Costs2", rank=Rank.BG)
    b.deploy_wind = 2
    p.hand.extend([a, b])
    ...
    end_of_turn(gs)
    # flips to B and draws; start_of_turn runs automatically
